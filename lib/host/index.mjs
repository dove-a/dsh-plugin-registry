'use strict'
// @dove-a/dsh-plugin-registry — Host 侧 Cordis 插件
// 形态对齐官方先例 @linxin666/dsh-ssh（lib/index.js）：
...
function pluginRegistryListTool() {
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        if (!value.ok) return `registry: ${value.error || 'unknown error'}`
        const entries = value.entries ?? []
        const byType = {}
        for (const e of entries) byType[e.type] = (byType[e.type] ?? 0) + 1
        const lines = entries.map((e) => `${e.type === 'plugin' ? '插件' : e.type === 'preset' ? '预设' : '技能'} ${e.name}@${e.version || '—'} | ${e.carrier} | ${e.path}`)
        return [`共 ${entries.length} 条记录（${Object.entries(byType).map(([k, v]) => `${k}: ${v}`).join(', ')}）`, ...lines].join('\n')
      }
    },
    async execute() {
      return runCli(['list'])
...
function pluginRegistryCheckTool() {
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        if (!value.ok) return `检查失败: ${value.error || 'unknown error'}`
...
function pluginRegistryReportTool() {
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        if (!value.ok) return `report: ${value.error || 'unknown error'}`
...
const GUIDANCE = [
  '本机已安装 @dove-a/dsh-plugin-registry 插件（DSH 插件路径注册表）：',
